...
def correct_input(prompt, op1, op2):
    while True:
        decision = input(prompt).lower()
        if op1 in decision:
            break
        elif op2 in decision:
            break
        else:
...
def alien_crater():
    if "space_weapon" in your_list:
        ...
        print_pause("By scaring the " + random_alien + ",")
        print_pause("you have rid the compound of the creature for good!")
...
def try_again():
    ...
    if "yes" in decision:
        ...
        print_pause("Live long and prosper.")
